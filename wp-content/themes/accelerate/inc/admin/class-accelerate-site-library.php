<?php
/**
 * Accelerate_Site_Library class to fetch the
...
class Accelerate_Site_Library {
	 ...
	 *
	 * Accelerate_Site_Library constructor.
	 */
	public function __construct() {
		$this->accelerate_site_library_demo_packages();
	}

	/**
	 * Demo config packages
	 */
	public function accelerate_site_library_demo_packages() {
		$this->accelerate_site_library_get_demos();
	}
	 ...
	 */
	public static function accelerate_site_library_get_demos() {
		$template = 'accelerate';
		$packages = get_transient( 'accelerate_site_library_theme_' . $template );
		...
		if ( ! is_wp_error( $raw_packages ) ) {
			...
			if ( $packages ) {
				set_transient( 'accelerate_site_library_theme_' . $template, $packages, WEEK_IN_SECONDS );
			}
		}
	 ...
	 */
	public static function accelerate_site_library_page_content() {
		$template      = 'accelerate';
		$output        = '';
		$demo_packages = self::accelerate_site_library_get_demos();
		...
		if ( isset( $demo_packages ) ) :
			...
			$output .= '<h2 class="screen-reader-text hide-if-no-js">' . esc_html__( 'Themes list', 'accelerate' ) . '</h2><!-- .screen-reader-text.hide-if-no-js -->';
			...
			foreach ( $demo_packages->demos as $demo_package_demo => $demo_package_data ) {
				...
				if ( isset( $demo_package_data->isPro ) ) {
					...
					$output .= esc_html__( 'Pro', 'accelerate' );
				...
				$output .= esc_html__( 'Preview', 'accelerate' );
				$output .= '</a>';
				...
				if ( isset( $demo_package_data->isPro ) ) {
					...
					$output .= esc_html__( 'Buy Now', 'accelerate' );
