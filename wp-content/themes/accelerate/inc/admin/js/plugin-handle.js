...
jQuery( document ).ready( function ( $ ) {
	...
	$( '.btn-get-started' ).click( function ( e ) {
		...
		// Change button text.
		jQuery( this ).text( accelerate_redirect_demo_page.btn_text );
		...
		var data = {
			...
			security : accelerate_redirect_demo_page.nonce,
			state    : state
		};
		...
		$.ajax( {
			...
			success : function ( response ) {
				extra_uri = '';
				if ( jQuery( '.accelerate-message-close' ).length ) {
					dismiss_nonce = jQuery( '.accelerate-message-close' ).attr( 'href' ).split( '_accelerate_notice_nonce=' )[1];
					extra_uri     = '&_accelerate_notice_nonce=' + dismiss_nonce;
				}
