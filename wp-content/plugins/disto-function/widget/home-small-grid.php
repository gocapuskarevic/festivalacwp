...
class home_ajax_grid_small_three_col_widget extends WP_Widget {
    ...
    public function widget( $args, $instance ) {
        ...
        ?>
<div class="jelly_homepage_builder jl_nonav_margin homepage_builder_3grid_post <?php echo esc_attr($font_size);?> <?php if($number_col=='col2'){echo " jl_cus_grid2 ";}elseif($number_col=='col4'){echo "jl_cus_grid4 ";}elseif($number_col=='col5'){echo "jl_cus_grid5 ";}else{echo "jl_cus_grid3 ";}?> <?php if($style_col=='style3'){echo " col_style3 ";}elseif($style_col=='style2'){echo " colstyle2 ";}else{echo " colstyle1 ";}?> <?php if($jl_hide_author == 1){echo 'jl_hide_author';}else{}?> <?php if($jl_hide_author_img == 1){echo 'jl_hide_author_img';}else{}?> <?php if($jl_hide_date == 1){echo 'jl_hide_date';}else{}?>">
        ...
        if(get_theme_mod('disable_post_category') !=1){
          ...
          if ($categories) {
            ...
            echo "</span>";
                ...
                <p><?php $content = wp_trim_words( get_the_content(), 19, '...' );
                    $regex = "@(https?://([-\w\.]+[-\w])+(:\d+)?(/([\w/_\.#-]*(\?\S+)?[^\.\s])?)?)@";
                    echo preg_replace($regex, ' ', $content);?></p>
                </div>
                <?php }else{}?>                                        
                </div>
                </div>
...
if($number_col=='col2'){
    ...
    if($row_count %3==0){echo '<div class="clear_line_3col_home"></div>';}
            ...
            wp_add_inline_script( 'disto-custom', "(function($){ $(document).ready(function() {'use strict'; var current_page_".esc_js($unique_block_id)." = 1; $('.jl-post-block-".esc_js($unique_block_id)." .jl_btn_load').click(function(e){ e.preventDefault(); e.stopPropagation(); var button = $(this), data = {'action': 'jl_post_more','query': ".json_encode( $posts_query->query_vars , true).",'page' : current_page_".esc_js($unique_block_id).",'cat' : '".esc_js($cats)."','offset' : '".esc_js($number_offset)."','jl_layout' : 'postsgrid','post_exception' : '".esc_js($post_exception)."','post_cat_none' : '".esc_js($post_cat_none)."','show_post_format' : '".esc_js($show_post_format)."','number_col' : '".esc_js($number_col)."'}; var button_default_text = button.text(); $.ajax({ url : '".esc_url(site_url())."/wp-admin/admin-ajax.php', data : data, type : 'POST', beforeSend : function ( xhr ) {button.text('');button.addClass('btn-loading'); }, success : function( data ){ if( data ) { button.text( button_default_text ); button.removeClass('btn-loading'); $('.jl-post-block-".esc_js($unique_block_id)." .jl-loadmore-btn-w').before(data); current_page_".esc_js($unique_block_id)."++; if ( current_page_".esc_js($unique_block_id)." == ".esc_js($posts_query->max_num_pages)." ) button.remove(); }else {button.remove();}}});});});})(jQuery);");
            }
            wp_reset_postdata();
            ?>
        </div>
...
<p class="jlwidget_heading"><?php esc_html_e('Grid Style', 'disto'); ?></p>
<p>
